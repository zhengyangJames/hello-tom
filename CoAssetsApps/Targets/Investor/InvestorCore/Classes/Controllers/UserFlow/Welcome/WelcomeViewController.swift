...
class WelcomeViewController: BaseViewController {
    ...
    private var heightForRow = (Double(UIScreen.mainScreen().bounds.height) -
        AppDefine.ScreenSize.Navigation)/2
    ...
    private func setupNavigation() {
        ...
        let setting = UIBarButtonItem(image: newIcon!,
            style: UIBarButtonItemStyle.Plain, target: self, action: "actionSetting:")
    ...
    private func setupTableview() {
        ...
        tableview.registerNib(UINib(nibName: WelcomeCell.className, bundle: nil),
            forCellReuseIdentifier: WelcomeCell.className)
    }
}
...
extension WelcomeViewController {
    var dataArray: NSArray? {
        guard let array = ResourcesHelper.readPlistFile("WelcomePlist") else {
            return []
        }
    ...
    func rotated() {
        if UIDeviceOrientationIsLandscape(UIDevice.currentDevice().orientation) {
            NSOperationQueue.mainQueue().addOperationWithBlock({() -> Void in
                self.heightForRow = (Double(UIScreen.mainScreen().bounds.height) -
                    AppDefine.ScreenSize.Navigation)/2
...
extension WelcomeViewController: UITableViewDataSource {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if let cell = tableview.dequeueReusableCellWithIdentifier(WelcomeCell.className, forIndexPath: indexPath) as? WelcomeCell {
