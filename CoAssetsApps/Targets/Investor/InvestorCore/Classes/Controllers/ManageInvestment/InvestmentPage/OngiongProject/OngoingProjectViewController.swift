...
extension OngoingProjectViewController {
    var dataArray: NSArray? {
        guard let array = ResourcesHelper.readPlistFile("OngingProjectPlist") else {
            return []
        }
...
extension OngoingProjectViewController: UITableViewDataSource {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if let cell = tableview.dequeueReusableCellWithIdentifier(OngoingProjectCell.className, forIndexPath: indexPath) as? OngoingProjectCell {
