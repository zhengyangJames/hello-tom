...
class OngoingProjectDetailViewController: BaseViewController {
    ...
    private func setupUI() {
        ...
        tableview.registerNib(UINib(nibName: OngoingProjectDetailCell.className, bundle: nil),
            forCellReuseIdentifier: OngoingProjectDetailCell.className)
...
extension OngoingProjectDetailViewController {
    private var dataArray: NSArray? {
        guard let array = ResourcesHelper.readPlistFile("NaThaiEstatePlist") else {
            return []
        }
...
extension OngoingProjectDetailViewController {
    private func ongoingProjectDetailCell(tableView: UITableView,
        cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCellWithIdentifier(OngoingProjectDetailCell.className,
            forIndexPath: indexPath) as? OngoingProjectDetailCell, myData = dataArray {
